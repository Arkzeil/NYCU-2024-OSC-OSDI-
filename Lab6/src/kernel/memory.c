...
void init_allocator()
{
    ...
    */
    ...
    memory_reserve2(0x0, 0x1000); // spin tables
    memory_reserve2(PHYS_TO_VIRT(MMU_PGD_ADDR), PHYS_TO_VIRT(MMU_PTE_ADDR+0x2000)); // // PGD's page frame at 0x1000 // PUD's page frame at 0x2000 PMD 0x3000-0x5000
    memory_reserve2((unsigned long long)&_kernel_start, (unsigned long long)&_kernel_end); // kernel
    memory_reserve2((unsigned long long)&_stack_end, (unsigned long long)&_stack_top);  // heap & stack -> simple allocator
    memory_reserve2((unsigned long long)cpio_addr, (unsigned long long)cpio_end);
}
...
void memory_reserve2(unsigned long long start, unsigned long long end)
{
